...
class ItsmTaskIntegrationTest extends XLReleaseIntegrationScalaTest {
  ...
  describe("ItsmTask") {
    it("should invoke facet API when create_task is successful") {
      ...
      executeAndVerifyFacet(task = task,
        ...
        serverUser = "xlr",
        recordNumber = "CHG0030696",
        recordUrl = s"http://localhost:${wireMockServer.port()}/change_request.do?sys_id=439c0e4fdb997300cab6ee82ca96196b",
    ...
    it("should invoke facet API when update_task is successful") {
      ...
      executeAndVerifyFacet(task = task,
        serverUrl = s"http://localhost:${wireMockServer.port()}",
        serverUser = "xlr",
        recordNumber = "INC0010340",
        recordUrl = s"http://localhost:${wireMockServer.port()}/incident.do?sys_id=a5ac06cfdb997300cab6ee82ca96191f",
    ...
    it("servicenow.CheckChangeRequest") {
      ...
      executeAndVerifyFacet(task = task,
        serverUrl = s"http://localhost:${wireMockServer.port()}",
        serverUser = "xlr",
        recordNumber = "CHG0030700",
        recordUrl = s"http://localhost:${wireMockServer.port()}/change_request.do?sys_id=1ecc46cfdb997300cab6ee82ca961944",
    ...
    it("servicenow.CheckStatus") {
      ...
      executeAndVerifyFacet(task = task,
        serverUrl = s"http://localhost:${wireMockServer.port()}",
        serverUser = "xlr",
        recordNumber = "CTASK0010260",
        recordUrl = s"http://localhost:${wireMockServer.port()}/change_task.do?sys_id=34bc0e4bdbd97300b5326ce2ca96190d",
    ...
    it("servicenow.FindRecordByTicket") {
      ...
      executeAndVerifyFacet(task = task,
        ...
        serverUser = "xlr",
        recordNumber = "CHG0030696",
        recordUrl = s"http://localhost:${wireMockServer.port()}/task.do?sys_id=439c0e4fdb997300cab6ee82ca96196b",
        title = "short description from XL Release",
        status = "New",
        priority = "3 - Moderate",
        createdBy = "xlr")
    ...
    it("servicenow.PublishArticle") {
      ...
      executeAndVerifyFacet(task = task,
        serverUrl = s"http://localhost:${wireMockServer.port()}",
        serverUser = "xlr",
        recordNumber = "KB0010134",
        recordUrl = s"http://localhost:${wireMockServer.port()}/kb_view.do?sys_kb_id=f583cfd3db95b300cab6ee82ca96193b",
  ...
  private def executeAndVerifyFacet(task: CustomScriptTask,
    ...
    facet.getProperty[String]("recordNumber") shouldBe recordNumber
    facet.getProperty[String]("recordUrl") shouldBe recordUrl
