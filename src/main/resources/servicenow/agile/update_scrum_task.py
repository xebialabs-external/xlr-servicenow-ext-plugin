...
class ServiceNowUpdateScrumTaskClient(object):
    ...
    def print_links(self, sys_id, ticket, data):
        mdl.println("Updated scrum task '{}' with number '{}' in Service Now. \n".format(ticket, data['number']))
        mdl.print_hr()
...
data = ServiceNowUpdateScrumTaskClient(locals()).process()