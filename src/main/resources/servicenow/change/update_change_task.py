...
class ServiceNowUpdateChangeTaskClient(object):
    ...
    def print_links(self, sys_id, ticket, data):
        mdl.println("Update change task '{}' with number '{}' in Service Now. \n".format(ticket, data['number']))
        mdl.print_hr()
...
data, number = ServiceNowUpdateChangeTaskClient(locals()).process()