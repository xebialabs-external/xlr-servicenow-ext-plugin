...
class ServiceNowRecordClient(object):
    ...
    def process(self):
        ...
        sys_id, ticket = response['target_sys_id'], response['target_record_number']
        data = self.sn_client.find_record(table_name=self.table_name, query="number={}".format(ticket))[0]
        self.print_links(sys_id, ticket, data)
        return sys_id, ticket, data
